...
# --only=salus-once --only=salus-nodefault --only=salus-default -only=sbi-index
...
def salus_dependencies():
    ...
    #
    # We need 4 variations on the spin module
    #
    crates_repository(
        name = "salus-rwlock",
        isolated = False,
        cargo_lockfile = "@salus//bazel-locks:Cargo.Bazel-rwlock.lock",
        lockfile = "@salus//bazel-locks:cargo-bazel-rwlock-lock.json",
        packages = {
            "spin": crate.spec(
                version = "*",
                default_features = False,
    ...
    crates_repository(
        name = "salus-once",
        isolated = False,
        cargo_lockfile = "@salus//bazel-locks:Cargo.Bazel-once.lock",
        lockfile = "@salus//bazel-locks:cargo-bazel-once-lock.json",
        packages = {
            "spin": crate.spec(
                version = "*",
                default_features = False,
                features = ["once", "spin_mutex"],
            ),
        },
    )

    crates_repository(
        name = "salus-nodefault",
        isolated = False,
        cargo_lockfile = "@salus//bazel-locks:Cargo.Bazel-nodefault.lock",
        lockfile = "@salus//bazel-locks:cargo-bazel-nodefault-lock.json",
        packages = {
            "spin": crate.spec(
                version = "*",
                default_features = False,
    ...
    crates_repository(
        name = "salus-default",
        isolated = False,
        cargo_lockfile = "@salus//bazel-locks:Cargo.Bazel-default.lock",
        lockfile = "@salus//bazel-locks:cargo-bazel-default-lock.json",
        packages = {
            "spin": crate.spec(
                version = "*",
